...
Hmm, if we wanted to analyze every neighborhood we would need to write over 250 lines of code that look very similar. We can do better with *loops*. Let's see how we can do this for the first 20 neighborhoods with a `for` loop:

```{r}
# remember that we have got the neighborhood names already
for (neighborhood in neighborhoods[1:20]) {
...
```{r, results="hide"}
sapply(neighborhoods, analyze_neighborhood)
```
